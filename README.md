...
```mermaid
flowchart TD
    A[Semiconductor Equipment] --> B[Equipment Data Collection]
    B --> C[SECS/GEM / OPC UA / MQTT]

    C --> D[Manufacturing Data Platform]
    D --> E[Wafer / Lot / Tool Context]

    E --> F[Metrology & Inspection Data]
    F --> G[SPC Engine]

    G --> H[Control Charts]
    G --> I[Process Capability]
    G --> J[Run Rules]

    H --> K[Excursion Detection]
    I --> K
    J --> K

    K --> L[Fault Detection & Classification]
    L --> M[Root Cause Analysis]

    M --> N[Process Engineering Decision]
    N --> O[Corrective Action]

    O --> P[Recipe / Process Adjustment]
    P --> Q[Closed-Loop Process Control]

    Q --> R[Yield Improvement]
```
...
```mermaid
flowchart LR
    A[Fab Equipment] --> B[OPC UA / SECS-GEM]
    B --> C[Kafka / MQTT]
    ...
    G --> I[pyspc / mfgQC / Cassini]
    H --> J[PyOD / scikit-learn]
    H --> K[XGBoost / PyTorch]

    I --> L[Excursion Detection]
    J --> L
    K --> M[Predictive Process Control]

    L --> N[Root Cause Analysis]
    M --> N

    N --> O[Engineering Workflow]
    O --> P[APC / MES]

    E --> Q[Grafana / Superset]
    F --> Q
```
...
```mermaid
flowchart LR
    A[Collect Equipment Data] --> B[Normalize Data]
    B --> C[Associate Wafer / Lot / Tool]

    C --> D[Collect Metrology]
    D --> E[Build SPC Baseline]

    E --> F[Monitor Process]
    F --> G{Excursion?}

    G -->|No| H[Continue Production]
    G -->|Yes| I[Alarm / Investigation]

    I --> J[FDC / Correlation Analysis]
    J --> K[Root Cause]

    K --> L[Corrective Action]
    L --> M[Recipe / Process Adjustment]

    M --> N[Validate Process]
    N --> O[Update Control Limits]
